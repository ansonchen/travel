...
var ThinkAjax = {
	...
	updateTip:'<div class="notification information png_bg" ><div>数据处理中...</div></div>',	// 后台处理中提示信息
	updateEffect:{'opacity': [0.1,0.85]},			// 更新效果
	image:['','',''], // 依次是处理中 成功 和错误 显示的图片
	cInfoWarp:['<span>','</span>'],//处理中的html结构
	sInfoWarp:['<span style="color:blue">','</span>'],//成功处理的html结构
	eInfoWarp:['<span style="color:red">','<span>'],//错误处理的html结构
	iInfoWarp:['<span>','</span>'],//服务信息中的html结构
	...
	loading:function (target,tips,effect){
		if ($(target))
		{
			...
			*/
			...
			$(target).innerHTML = this.cInfoWarp[0] + tips + this.cInfoWarp[1];
			//使用更新效果
	...
	ajaxResponse:function(request,target,response){
      ...
      try{
            ...
            {
                $(target).innerHTML    =  this.iInfoWarp[0] + "服务器返回数据出错!" + this.iInfoWarp[1];
...
/*					if ('' != this.image[1])
					...
					$(target).innerHTML	= this.sInfoWarp[0] + this.info + this.sInfoWarp[1];
				...
				*/
				...
				$(target).innerHTML	= this.eInfoWarp[0] + this.info + this.eInfoWarp[1];
